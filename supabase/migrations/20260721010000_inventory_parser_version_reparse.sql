drop index public.inventory_snapshots_world_success_hash_idx;
...
comment on index public.inventory_snapshots_world_success_parser_idx is
  'Successful inventory snapshots are idempotent for identical source bytes and parser identity; a parser upgrade may publish a corrected immutable projection.';

create or replace function private.get_latest_inventory_snapshot_for_agent(p_world_id uuid)
returns jsonb
language sql
stable
security definer
...
comment on function private.publish_inventory_snapshot(uuid, jsonb) is
  'Atomically publishes one immutable canonical inventory projection per source hash and parser identity.';
