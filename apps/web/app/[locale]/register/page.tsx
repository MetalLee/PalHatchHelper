...
export default async function RegisterPage({
  ...
  if (!isAppLocale(locale) || !isPasswordLoginEnabled()) notFound();
  const [brandCopy, register] = await Promise.all([
    getTranslations({ locale, namespace: "Brand" }),
    getTranslations({ locale, namespace: "Register" }),
  ]);
  ...
  return (
    <main className="relative min-h-dvh overflow-x-hidden">
      ...
      <div className="relative z-10 mx-auto grid min-h-dvh w-full max-w-[90rem] items-center gap-8 px-4 py-6 sm:px-6 sm:py-10 lg:grid-cols-[minmax(0,1fr)_minmax(26rem,31rem)] lg:gap-14 lg:px-10">
        ...
        <section className="my-16 w-full max-w-[31rem] justify-self-center rounded-[2rem] border border-glass-border bg-white/74 p-5 shadow-soft backdrop-blur-xl sm:p-8 lg:my-0 lg:justify-self-end">
