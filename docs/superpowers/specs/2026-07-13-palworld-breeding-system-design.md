...
4. 帕鲁种类必须能映射到目录数据；未知值记录告警。
   库存来源中以 `boss_` 开头的头目个体先去除一层前缀，再以剩余部分作为基础帕鲁稳定 ID；
   原始内部名保留在筛选后的 `raw_metadata` 中用于审计。去除前缀后仍无法映射时才记录
   `UNKNOWN_PAL`。
...
7. 所有权分为 `player`、`guild` 和 `unresolved`。有效玩家 UID 映射为 `player`；位于基地、
   ...
   `unresolved`。只有 `unresolved` 不进入共享池。
8. 库存数量异常下降时进入待审核状态。
...
20. Boss 前缀库存 ID 标准化、公会所有基地帕鲁解析、列表展示、配种可用性和计划生命周期测试。
