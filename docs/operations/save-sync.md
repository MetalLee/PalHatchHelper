...
9. Parser 在独立进程中运行。Landlock 只允许读取系统运行时、显式 Parser 运行文件和快照，只允许写临时输出目录；seccomp 禁止网络、权限修改和 `fork`/`vfork`/`clone`/`clone3`，另有单 CPU affinity、内存/CPU/单文件与目录总输出大小、PID 和超时限制。独立 Compose service 再以 cgroup 限制 1 CPU、2 GB 内存和 32 PID，并用 64 MB tmpfs 限制 Parser 运行输出。
