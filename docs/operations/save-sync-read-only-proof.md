...
- `settings.py`：`PALWORLD_SAVE_ROOT` 无默认值；生产 readiness 要求绝对、显式配置。
...
- Landlock 将文件写入限制到临时输出目录，seccomp 禁止网络 socket 与 chmod；子进程环境是白名单，不继承 Supabase key、代理或其他进程环境。
- Repository payload 不含原始路径或原始存档，数据库 RPC 只接收 CanonicalSnapshot 标准化字段。
- 任一失败路径都不调用发布 RPC，Supabase `latest_snapshot_id` 保持上一有效值。
...
- `tests/parsers/test_adapter_contract.py` 覆盖超时、非零退出、非法 JSON、快照只读、网络禁用、秘密环境隔离和输出目录外写入拒绝。
- `tests/normalization` 与 `tests/save_sync/test_publish_guard.py` 覆盖 world UID、重复实例、未知 ID 告警、unresolved、骤降与失败不发布。
- `supabase/tests/inventory_sync.sql` 覆盖单事务发布、哈希幂等、数据库侧 world UID/骤降复核与上一 latest 保留。
