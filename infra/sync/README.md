...
只有 `palbeacon-cli@0.2.0` 已按发布手册人工发布并完成 tgz 校验后，才安装精确
版本；不要使用 `latest`：

```bash
sudo npm install --global --ignore-scripts=false palbeacon-cli@0.2.0
/usr/local/bin/palbeacon --version
```

版本输出必须为 `0.2.0`。生产安装不从本地重新打包另一个 tarball。
